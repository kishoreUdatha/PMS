"""Database wiring for the booking-core service."""
...
engine = make_engine(settings.booking_database_url)
...
def get_session(request: Request) -> Iterator[Session]:
    ...
    Critical for the inventory path: the whole hold request runs in a single
    transaction so ``SELECT FOR UPDATE`` locks are held until commit/rollback.
    """
